...
import { getAllNotebookSummaries, getAllWriteups, getWriteupBySlug } from '@/lib/content-loader'
      ...
      <div className="grid gap-12 lg:grid-cols-[280px_minmax(0,1fr)]">
        ...
        <article className="rounded-[2rem] border border-white/10 bg-slate-950/72 p-8 shadow-[0_24px_80px_rgba(2,8,23,0.32)] backdrop-blur sm:p-10">
          <div className="mb-8 space-y-4 border-b border-white/10 pb-8">
            <p className="text-sm uppercase tracking-[0.25em] text-cyan-300/80">{post.date}</p>
            <h1 className="max-w-3xl text-4xl font-semibold tracking-tight text-white sm:text-5xl">
              {post.title}
            </h1>
            <p className="max-w-2xl text-base leading-7 text-slate-300">{post.description}</p>
